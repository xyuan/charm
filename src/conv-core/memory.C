...
{ /* We're out of memory: free up the liferaft memory and abort */
  ...
  if (nBytes>0) CmiAbortf("Could not malloc() %d bytes--are we out of memory? (used :%.3fMB)",nBytes,CmiMemoryUsage()/1000000.0);
  else CmiAbortf("Could not malloc()--are we out of memory? (used: %.3fMB)", CmiMemoryUsage()/1000000.0);
}
