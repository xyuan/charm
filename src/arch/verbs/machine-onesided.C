...
void postRdma(
  ...
  if(context->tokensLeft<0){
    CmiAbortf("No remaining tokens! Pass a larger value for maxTokens (%d) using argument +IBVMaxSendTokens\n", maxTokens);
  }
#endif
  ...
  if (retval = ibv_post_send(node->infiData->qp, &wr, &bad_wr)) {
    ...
    CmiAbortf("ibv_post_send failed in postRdma!! Try passing a larger value for maxTokens (%d) using argument +IBVMaxSendTokens\n",maxTokens);
  }
}
